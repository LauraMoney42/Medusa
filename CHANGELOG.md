## 2026-09-18 14:45
- Fix: `resolveWorkingDir` in `server/src/routes/sessions.ts` now expands a leading `~` or `~/` to `os.homedir()` before the absolute/relative resolution, matching shell semantics. Previously a tilde path (which is what the New Chat modal's own placeholder shows) resolved to the literal `<home>/~/...`, which does not exist, and the API returned "Invalid working directory"
- Tests: added two route tests in `server/src/routes/__tests__/sessions.test.ts` (tilde-prefixed folder resolves to the real path; bare `~` resolves to home). `npx vitest run`: 38 files, 545 tests pass. `npx tsc --noEmit` reports 5 pre-existing errors in `src/mcp/medusa-mcp-shim.ts` and `src/runner/runner-client.ts` caused by `@modelcontextprotocol/sdk` and `socket.io-client` not being installed in this worktree's node_modules; identical with and without this change
- Files affected: server/src/routes/sessions.ts, server/src/routes/__tests__/sessions.test.ts
