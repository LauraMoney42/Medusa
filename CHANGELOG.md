## 2026-09-18 17:23
- Fix: Live voice closed with 1008 the moment Medusa started answering. The persona told the realtime model it had Read, Edit, and shell tools, so Gemini tried an undeclared ls tool and the service dropped the socket. buildOrchestratorPrompt gains liveTools: a "Your tools" contract naming only the five declared functions and routing all file and shell work through spawn_agent. A service-side close now gets one reconnect before the chat drops to the local pipeline
- Files affected: server/src/sessions/orchestrator-prompt.ts, server/src/voice/live/live-session.ts, server/src/sessions/__tests__/orchestrator-prompt.test.ts

## 2026-09-18 17:15
- W8: Providers settings tab. Until now, an API key for OpenRouter, Gemini (Live voice), OpenAI or Deepgram could only be set by hand-editing `providers.<id>.apiKey` in `~/.claude-chat/settings.json`, or via an env var; Settings > Voice just told the owner to go edit the file by hand, which is how the owner's attempt to enable Gemini Live silently did nothing (the key was never actually stored anywhere).
- Server: `server/src/settings/providers.ts` gained `setProviderApiKey`/`removeProviderApiKey` (atomic write, `chmod 600`, same pattern as `settings/store.ts`), `getKeyStatus`/`resolveManagedProviderKey`, `listManagedKeyProviders` (OpenRouter plus the external services gemini/openai/deepgram in one list), and `verifyProviderKey` (one cheap authenticated call per provider: OpenRouter and OpenAI `GET /v1/models`, Gemini `GET /v1beta/models?key=...` with a check for a native-audio live model in the response, Deepgram `GET /v1/projects`). `server/src/routes/providers.ts` gained `GET /api/providers/keys`, `PUT`/`DELETE /api/providers/:id/key` (zod-validated body), and `POST /api/providers/:id/verify`, rate limited to 10 calls/minute per caller. No route ever returns a full key, only whether one is set, its source (settings vs env), and its last 4 characters.
- Confirmed (not built new, since it already worked this way): `getProviderApiKey`/`getExternalApiKey` read `~/.claude-chat/settings.json` fresh on every call rather than caching anything at import, and `getRealtimeProvider`/`listRealtimeProviders` (`server/src/voice/providers.ts`) are called per `voice:start`, not once at boot. A key saved through the new API is therefore already live for the very next voice session with no server restart; added `server/src/voice/__tests__/providers-realtime-key.test.ts` to prove that specific claim rather than assume it.
- Client: new `client/src/components/Settings/ProvidersTab.tsx`, first tab in Settings — one card per provider with a status pill (Set via settings / Set via env / Not set), a masked key field, Save, Verify (shows the result inline, including "Live audio model available" for Gemini), Remove, and a "Get a key" link. `client/src/api.ts` gained `fetchProviderKeys`/`saveProviderKey`/`removeProviderKey`/`verifyProviderKey`. New `client/src/stores/settingsTabStore.ts` lets another component ask the Settings modal to open on a specific tab; `VoiceTab.tsx`'s old "add it as `providers.gemini.apiKey` in `~/.claude-chat/settings.json`" text is now a real "Add your Gemini key in Providers" button. `VoiceTierBadge.tsx` shows a small banner under the mic badge when it reads Local and no realtime provider has a key at all, linking to the same place.
- Tests: `server/src/routes/__tests__/provider-keys.test.ts` (store/remove/mask, verify with mocked fetch including the Gemini native-audio-model check, and the 10/minute rate limit), `server/src/settings/__tests__/provider-key-precedence.test.ts` (settings beats env, both for the OpenRouter-style and external-service key paths), `server/src/voice/__tests__/providers-realtime-key.test.ts` (`GeminiLiveProvider` goes from not-ready to ready to not-ready again as the key is saved/removed at runtime, same process). All new tests isolate `HOME` to a temp directory; nothing here touches the owner's real settings file. `cd server && npx tsc --noEmit` clean, `npx vitest run` 698 passed (679 on main plus 19 new); 4 pre-existing failures in `src/socket/__tests__/voice-handlers.test.ts` reproduce identically on an unmodified checkout of this worktree and are unrelated to this change. `cd client && npx tsc -b --noEmit` clean, `npm run build` clean.
- Files affected: server/src/settings/providers.ts, server/src/routes/providers.ts, server/src/routes/__tests__/provider-keys.test.ts, server/src/settings/__tests__/provider-key-precedence.test.ts, server/src/voice/__tests__/providers-realtime-key.test.ts, client/src/api.ts, client/src/components/Settings/ProvidersTab.tsx, client/src/components/Settings/VoiceTab.tsx, client/src/components/Sidebar/SettingsModal.tsx, client/src/components/Sidebar/Sidebar.tsx, client/src/components/Voice/VoiceTierBadge.tsx, client/src/stores/settingsTabStore.ts, CHANGELOG.md

## 2026-09-18 17:12
- Fix: Live voice demoted to Local within a second because the Kokoro voice id (af_heart) was sent to Gemini as the speaker name and the Live API closed the socket with 1007. Realtime sessions now use a separate liveVoice setting validated against the Gemini voice list (default Aoede); Settings > Voice gains a Speaker select. Voice-handler tests pinned to the pipeline tier so a real Gemini key in settings.json cannot change their outcome
- Files affected: server/src/voice/live/gemini-live.ts, server/src/voice/live/live-session.ts, server/src/voice/providers.ts, server/src/voice/realtime.ts, server/src/packs/schema.ts, server/src/voice/live/__tests__/gemini-voice.test.ts (new), server/src/socket/__tests__/voice-handlers.test.ts, server/src/packs/__tests__/store.test.ts, client/src/api.ts, client/src/components/Settings/VoiceTab.tsx

## 2026-09-18 15:59
- Fix (owner, desktop app, speakers, always-on mic, Kimi warm engine): "working but still slow to think and reply. Buggy. Sometimes shows no response. Once I heard the same reply twice with a slight delay and overlap." The owner's `GET /api/voice/status?events=1` ring buffer was empty at the time this was investigated (no voice session was being tracked by the sidecar), so the two bugs were reproduced and fixed from a source audit plus new unit tests that exercise the exact sequences described, rather than from a captured trace.
- Root cause 1, the duplicate/overlapping reply: `voice:audio-chunk` only ever carried a per-turn `seq` that restarts at 0 for every `TtsStream`, with no id for the turn itself. The client's `GaplessAudioQueue` (client/src/lib/voice/audioScheduler.ts) never learned when a new turn began unless an explicit `voice:stop-audio` reset it, which a turn that ends normally never sends: `nextExpectedSeq` was simply left wherever the previous turn's last chunk put it. Also confirmed `resetSequence()` was never called anywhere in the client (the scheduler's own reset method was dead code) and `voice:speaking-start`/`voice:speaking-end` were not wired to anything on the client at all (no listener existed). Fix: `VoiceSession` now assigns a `turnId` per turn (`server/src/voice/session.ts`) and carries it on `voice:speaking-start`, `voice:audio-chunk`, `voice:speaking-end` and `voice:stop-audio`. `GaplessAudioQueue` gained `beginTurn(turnId)`, called from a new client wire-up of `voice:speaking-start` (previously unhandled); a genuine turn change hard-stops whatever the previous turn left playing or queued before anything from the new turn can be scheduled, and `enqueue` now drops any chunk tagged with a turn other than the current one.
- Root cause 2, the swallowed response: `VoiceSession` tracked abandoned (superseded) turns with a plain counter (`abandonedTurns++`/`--`), which is only correct if a stale end/error from an aborted turn always arrives before the live turn's own terminal event. When it instead arrived after (plausible any time an abort takes a beat to settle against the warm engine), the counter consumed the LIVE turn's own `message:stream:end` by mistake, so `TtsStream.end()` was never called and the reply was never spoken. Replaced the counter with real turn identity: `voice-handlers.ts`'s stream tap now forwards each event's own message id (`id`/`messageId`, already present on every `message:stream:*`/`message:error` payload) into `session.onStreamStart/onDelta/onStreamEnd/onStreamError`; `VoiceSession` compares it against the turn it is currently running and, when a turn is aborted, records its id as explicitly abandoned so a very late stale event is rejected even before the replacement turn has reached its own `message:stream:start`. This is order-independent by construction, unlike the counter.
- Also hardened: a reply that produced text but got no audio at all (every sentence's synthesis call failed) now logs `voice: reply had text but no audio (speech synthesis failed); showing text only` instead of failing silently; the text bubble was always rendered through the normal chat path regardless of voice, so this only fixes the missing explanation in the Activity Log, not a blank bubble.
- Speculation (S16 item 3) already restarted only on a materially different final transcript and logged the decision either way; left as is. Changed the "Answer before you finish" default: `speculation.enabled` in `server/src/socket/voice-handlers.ts` is now `speculativeStart && warmWanted` for the session's first `voice:start` (computed before `createSession`, not after), since jumping the gun buys nothing against a cold engine's ~10 s time to first token.
- Client: `client/src/types/voice.ts` gained `turnId` on the chunk/stop-audio payloads and new `VoiceSpeakingStartPayload`/`VoiceSpeakingEndPayload` types; `client/src/lib/voice/voiceBus.ts` gained `onSpeakingStart`/`onSpeakingEnd`/`emitSpeakingStart`/`emitSpeakingEnd`; `client/src/hooks/useSocket.ts` now subscribes to `voice:speaking-start`/`voice:speaking-end` (previously not listened to at all) and forwards them through the bus.
- Tests: `server/src/voice/__tests__/session.test.ts` gained three: a turn-ownership test that reproduces the swallowed-response bug directly (turn B's own end arrives before turn A's stale end, and the reply must still be spoken and reported), a turnId-propagation test, and a text-without-audio warning test. `server/src/voice/__tests__/session-speculation.test.ts`'s two abandoned-turn tests were updated to pass explicit turn ids (matching how the tap now really works) instead of relying on call order alone. `client/src/lib/voice/__tests__/audioScheduler.test.ts` gained five: `beginTurn` hard-stopping the previous turn, a foreign-turn chunk being dropped even with a fresh seq, a normally-ended turn not poisoning the next turn's seq-0 chunk (the literal "sometimes shows no response" scenario), `beginTurn` with the same id being a no-op, and a chunk with no `turnId` still working for backward compatibility.
- Verified: `cd server && npx tsc --noEmit` clean, `npx vitest run` 629 passed (626 on main plus 3 new in session.test.ts; the 2 updated speculation tests are not additional). `cd client && npx tsc -b --noEmit` clean, `npm run build` clean, and `node --test src/lib/voice/__tests__/audioScheduler.test.ts` 13 passed (8 on main plus 5 new). Live check on PORT=3990 with a temp HOME was not run this session: the sandbox refuses any command that sets `HOME` from inside a worktree-isolated agent (git-safety guard), which blocks standing up an isolated throwaway server without risking the owner's real `~/.claude-chat/sessions.json`. Coverage instead rests on the unit tests above, which reproduce the reported sequences directly.
- Files affected: server/src/voice/session.ts, server/src/socket/voice-handlers.ts, server/src/voice/__tests__/session.test.ts, server/src/voice/__tests__/session-speculation.test.ts, client/src/lib/voice/audioScheduler.ts, client/src/lib/voice/voiceBus.ts, client/src/lib/voice/__tests__/audioScheduler.test.ts, client/src/types/voice.ts, client/src/hooks/useSocket.ts, client/src/components/Voice/VoiceMicButton.tsx, CHANGELOG.md

## 2026-09-18 16:05
- S17 Live voice: true speech to speech. `server/src/voice/live/gemini-live.ts` implements `RealtimeVoiceProvider` over the Gemini Live API WebSocket (BidiGenerateContent), default model `gemini-2.5-flash-native-audio-latest`. 16 kHz PCM16 up, 24 kHz PCM16 down, server-side activity detection for barge-in, and both input and output transcription so the chat history stays complete
- The realtime model keeps Medusa's orchestration: its system instruction is `buildOrchestratorPrompt` with `voiceMode: true`, and every MCP tool (spawn_agent, agent_status, agent_result, list_agents, cancel_agent) is handed over as a Gemini function declaration and executed through `mcp/client.ts` against Medusa's own HTTP API with this chat as the parent session. The server bundle still imports no MCP SDK
- `server/src/voice/live/live-session.ts` dresses a realtime provider as the Section 7 contract, so the client is unchanged: the same `voice:state`, `voice:partial`, `voice:transcript`, `voice:audio-chunk`, `voice:stop-audio`, `voice:speaking-start/end` and `voice:latency` events. Audio is buffered to about 200 ms and re-wrapped as WAV, because the client scheduler decodes with `decodeAudioData`, which cannot read headerless PCM
- Tier selection: voice always works. `live` when a realtime key exists and the user has not pinned the pipeline, `pipeline` otherwise, announced once per start as a new `voice:tier {sessionId, tier, provider, model, reason}` event and an Activity Log line. A provider error mid-session demotes the chat to the local pipeline, speaks "Switching to local voice", and retries Live the next time voice is switched on
- Subagent follow-ups reach the live model: the existing broadcast tap spots a `message:user` carrying `source: "agent-followup"` and injects it as a turn, so she says it out loud. `subagents/followups.ts` is untouched
- Settings: `gemini` is registered as an external (non-chat) provider with `apiKeyEnv: GEMINI_API_KEY`, alongside `openai` and `deepgram`. Settings > Voice gained a Live voice card with an auto / pipeline-only / live-only tier select, provider and model selects, the tier badge, and the free-key hint linking to https://aistudio.google.com/apikey. A small Live/Local badge sits beside the mic
- `GET /api/voice/status` now reports `realtime.implemented: true` plus the tier, provider, model, key presence and reason a session would get right now
- NOT verified against the live service: there is no Gemini key on this machine, so the provider has never made a real call. The wire format is transcribed from the published BidiGenerateContent reference (ai.google.dev/api/live, /gemini-api/docs/live, /live-guide, /live-tools, read 2026-09-18) and exercised only through an injected fake socket
- Verified: `cd server && npx tsc --noEmit` clean, `npx vitest run` 676 passed (626 from main plus 50 new); `cd client && npx tsc -b --noEmit && npm run build` clean
- Files affected: server/src/voice/live/gemini-live.ts (new), server/src/voice/live/live-session.ts (new), server/src/voice/live/__tests__/gemini-live.test.ts (new), server/src/voice/live/__tests__/live-session.test.ts (new), server/src/socket/__tests__/voice-live-routing.test.ts (new), client/src/components/Voice/VoiceTierBadge.tsx (new), docs/LIVE_VOICE.md (new), server/src/voice/realtime.ts, server/src/voice/providers.ts, server/src/socket/voice-handlers.ts, server/src/socket/handler.ts, server/src/routes/voice.ts, server/src/routes/__tests__/voice.test.ts, server/src/settings/providers.ts, server/src/packs/schema.ts, server/src/packs/__tests__/store.test.ts, server/src/socket/__tests__/voice-handlers.test.ts, client/src/api.ts, client/src/types/voice.ts, client/src/stores/voiceStore.ts, client/src/stores/activityStore.ts, client/src/hooks/useSocket.ts, client/src/components/Voice/VoiceMicButton.tsx, client/src/components/Settings/VoiceTab.tsx, client/src/components/Activity/ActivityLogPanel.tsx, CHANGELOG.md
